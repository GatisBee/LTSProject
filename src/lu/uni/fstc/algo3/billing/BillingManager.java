...
public class BillingManager {
	...
	/**
	 * Creates bills for all vehicles that where using LTS during the specified
	 * year-month. There are no tests for the year month comparison, but it uses
	 * java API functionality for comparing so if the logic is correct, it
	 * should work.
	 *
	 * @param yearMonth
	 *            year and month for which the bills must be created.
	...
	public List<Bill> createMonthlyBills(YearMonth yearMonth) {
		...
		for (NumberPlate p : plates) {
			 ...
			 */
			List<ScanEntry> scansAllDirections = Filter.filterByNumberPlate(
					monthlyScans, p);
			/*
			For billing get only the inbound scans of number plate
			 */
			List<ScanEntry> scansInbound = Filter.filterByNumberPlate(
					scansAllDirections, p, Direction.IN);// in this case there
															// are redundant
															// checks on number
															// plate
			/*
	...
	/**
	 * Creates bills for all vehicles that where using LTS during the current
	 * year-month. This version of method is more or less for illustration
	 * purposes that the billing system works. It gets the scans for current
	 * month, sorts them, eliminates overlapping scans and then creates a bill
	 * for each number plate. For each traversal of a road section there is also
	 * check if the vehicle was not speeding and a penalty is added to the bill
	 * if it was. The criterion for determining speeding is time needed to
	 * traverse the road section without exceeding speed limit. It is up to the
	 * user to indicate an exact time or create a small "buffer" for minor
	 * speeding after which a penalty should be applied.
	 *
	...
	public List<Bill> createMonthlyBills() {
		 ...
		 */
		List<ScanEntry> thisMonthScans = Filter.filterByYearMonth(
				lts.getAllScans(), yearMonth);
		/*
		Sort this array by time.
		 */
		...
		for (NumberPlate p : plates) {
			 ...
			 */
			double ammountToPay = scansInbound.size() * pricePerSection; // global
																			// price
																			// for
																			// road
																			// section
	...
	/**
	 * Calculates penalties for speeding vehicles based on the time needed to
	 * travers a road section. This time is specified during the creation of a
	 * road section and can be adjusted. Speeding penalty is a global value that
	 * is applied to every speeding case.
	 * <p>
	 * NOTE that this method does not care about duplicate scans (e.g. 1 vehicle
	 * on different road sections at the same). Nor does it care if a vehicle
	 * enters a road section on the last day of month and exits on the first day
	 * of next month. It only calculates penalty for a case where a vehicle
	 * enters and exits (exactly in this sequence) the same road section on
	 * different checkpoints of that section. In the case of duplicate on
	 * another section the in/out sequence would be broken and those scans would
	 * be ignored. However if the duplicate appears on the same road section
	 * e.g. in->in->out->out only the middle sequence would be taken into
	 * account which could lead to a faulty speeding penalty. This means that
	 * prior to this method one should take care of possible duplicate situation
	 * by some policy of eliminating such duplicates.
	 * <p>
	 * I have tested this method by making sure that every vehicle will be
	 * speeding. In this case it was working.
	 *
	 * @param scans
	 *            scans of a single vehicle during some time period (month in
	 *            our case) for checking of speeding
	 * @return total penalty for speeding (sum of all offences). If no speeding
	 *         is detected method returns 0.0
	 */
	public double calculatePenalty(List<ScanEntry> scans) {
		...
		for (int i = 0; i < scans.size() - 1; i++) {
			 ...
			 */
			if (current.getDirection() == Direction.IN
					&& next.getDirection() == Direction.OUT
					&& !current.getCheckpoint().getName()
							.equals(next.getCheckpoint().getName())
					&& current.getRoadSection().getName()
							.equals(next.getRoadSection().getName())) {
				...
				Duration withoutSpeeding = Duration.ofMillis(current
						.getRoadSection().getTimeForCar());
